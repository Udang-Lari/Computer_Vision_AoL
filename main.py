...
def pre_process_landmark(landmark_list):
    ...
    for lm in landmark_list:
        temp.extend([lm.x - base_x, lm.y - base_y])
    return temp
...
def generate_frames():
    ...
    while True:
        ...
        if lmList:
            wrist_y = lmList[0][2]
            if wrist_y < h * 0.15:
                cv2.putText(frame, "IGNORED (TOO HIGH)", (20, 50),
                ...
                if ai_input:
                    ...
                    if class_id != 0 and prob > 0.90:
                        ...
                        if class_id == last_pred_class and detected != "IDLE":
                            ...
                            bx1 = int((w - BAR_WIDTH) / 2)
                            by1 = int(h * 0.75)

                            cv2.rectangle(frame,
                                          (bx1, by1),
                                          (bx1 + BAR_WIDTH, by1 + BAR_HEIGHT),
                                          (40, 40, 40), -1)
                            cv2.rectangle(frame,
                                          (bx1, by1),
                                          (bx1 + int(BAR_WIDTH * progress), by1 + BAR_HEIGHT),
                                          (0, 220, 120), -1)
        ...
        cv2.putText(frame, display_status, (20, 50),
